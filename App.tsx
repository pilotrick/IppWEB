...
const AppContent = () => {
  ...
  const handleCheckout = () => {
    ...
    const orderSummary = `SOLICITUD DE COTIZACIÓN (WEB ORDER):\n\n` + 
      cartItems.map(item => `- ${item.quantity.toLocaleString()}x ${item.name}`).join('\n') +
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans text-slate-900">
      {!isLoginPage && <Header cartCount={cartItems.reduce((acc, item) => acc + item.quantity, 0)} onOpenCart={() => setIsCartOpen(true)} />}
      {isAuthenticated && <CartDrawer isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} items={cartItems} onRemove={removeFromCart} onUpdateQuantity={updateQuantity} onCheckout={handleCheckout} />}
